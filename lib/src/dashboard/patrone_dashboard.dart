...
class _PatroneDashboardState extends State<PatroneDashboard>
  ...
  List<Widget> views(BuildContext context) {
    return [
      ...
      ProfileScreen(
        userProfile: currentUserPatroneProfile,
      ),
    ];
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          (confirmedPayment ?? false) ? (trailingIcon() ?? const SizedBox()) : SizedBox(),
        ],
      ),
      body: SafeArea(
        child: Column(
                ...
                child: StreamBuilder<DocumentSnapshot>(
                    ...
                    builder: (context, snapshot) {
                      if (snapshot.hasData) {
                        ...
                        bool isFreeTrial = !(userData['createdAt'] as Timestamp)
                            .toDate()
                            .add(Duration(days: 14))
                            .isBefore(DateTime.now());
                        if (isFreeTrial ||
                            (userData.containsKey('patrone_payment') 
                            && (userData['patrone_payment']['expiration_date'] as Timestamp).toDate().isAfter(DateTime.now()))) {
